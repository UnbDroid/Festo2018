...
namespace rec
{
	namespace grappler
	{
		...
		public:
			...
			{
				if (other.num_servos_found != num_servos_found) return true;
				if (other.servos.count() != servos.count()) return true;

				for (int i = 0; i < other.servos.count(); ++i)
				{
					if (other.servos[i] != servos[i]) return true;
				}
