...
namespace rec
{
	namespace grappler
	{
		...
		public:
			...
			bool operator!=(const ServoInfo& other) const
			{
				if (other.found != found) return true;
				if (other.channel != channel) return true;
				if (other.id != id) return true;
				if (other.currentPosition != currentPosition) return true;
				if (other.currentSpeed != currentSpeed) return true;
				if (other.error != error) return true;
				if (other.cwAxisLimit != cwAxisLimit) return true;
				if (other.ccwAxisLimit != ccwAxisLimit) return true;
